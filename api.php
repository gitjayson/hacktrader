...
function update_health_status($path, $ticker, $period, $liveStatus, $provider = null, $errorSummary = null, $cacheAgeSeconds = null) {
    $state = load_json_file($path, [
        'meta' => ['updated_at' => null, 'version' => 'v0.12.0'],
        'counters' => [
            'total_requests' => 0,
            'live_successes' => 0,
            'cache_hits' => 0,
    ...
    try {
        ...
        if (function_exists('current_user_record')) {
            ...
            if ($sub_user) {
                ...
                if (!$allowed) {
                    error_log("v0.12.0 quota: user {$sub_user['email']} over plan {$sub_user['plan']} call limit");
                    if ($hardGate) {
        ...
        error_log('v0.12.0 subscription gate failed (non-fatal): ' . $e->getMessage());
    }
}

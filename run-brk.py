...
secrets_path = next(
    (path for path in secrets_candidates if os.path.exists(path)), secrets_candidates[0]
)
...
def nearest_levels(levels, current, direction, count=2):
    if direction == "above":
        filtered = sorted(
            [lvl for lvl in levels if lvl["price"] > current], key=lambda lvl: lvl["price"]
        )
    else:
        filtered = sorted(
            [lvl for lvl in levels if lvl["price"] < current],
        ...
        channels.append(
            {
                ...
                "location": "inside"
                if support1["price"] <= current <= resistance1["price"]
                else ("above" if current > resistance1["price"] else "below"),
            }
        )
...
def score_breakout(
    ...
    drivers.append(
        {
            "factor": "channel_position",
            "value": round_maybe(position, 3),
            "impact": round_maybe(position_impact, 2),
        }
    )
    if previous_day:
        ...
        if pdh is not None:
            ...
            drivers.append(
                {
                    "factor": "previous_day_high",
                    "value": round_price(pdh),
                    "impact": round_maybe(impact, 2),
                }
            )
        if pdl is not None:
            ...
            drivers.append(
                {
                    "factor": "previous_day_low",
                    "value": round_price(pdl),
                    "impact": round_maybe(impact, 2),
                }
            )
    if recent:
        if recent.get("recent_high") is not None and current >= recent["recent_high"] - atr * 0.15:
            score += 6.0
            drivers.append(
                {"factor": "near_recent_high", "value": recent["recent_high"], "impact": 6.0}
            )
        if recent.get("recent_low") is not None and current <= recent["recent_low"] + atr * 0.15:
            score -= 6.0
            drivers.append(
                {"factor": "near_recent_low", "value": recent["recent_low"], "impact": -6.0}
            )
    ...
    if bar_ratio is not None:
        ...
        drivers.append(
            {
                "factor": "bar_volume_ratio",
                "value": round_maybe(bar_ratio, 2),
                "impact": round_maybe(impact, 2),
            }
        )
    if day_ratio is not None:
        ...
        drivers.append(
            {
                "factor": "day_volume_ratio",
                "value": round_maybe(day_ratio, 2),
                "impact": round_maybe(impact, 2),
            }
        )
    ...
    drivers.append(
        {
            "factor": "failed_up_attempts",
            "value": failed_up,
            "impact": round_maybe(up_fail_impact, 2),
        }
    )
    drivers.append(
        {
            "factor": "failed_down_attempts",
            "value": failed_down,
            "impact": round_maybe(down_fail_impact, 2),
        }
    )
    if failed_up >= 3:
        score -= 8.0
...
def calculate_indicators(rows):
    ...
    macd_line = [
        ...
        for e12, e26 in zip(ema12, ema26, strict=False)
    ]
...
def compute_output(ticker, interval, display, periods, values, source):
    ...
    eastern_time = (
        latest_dt_et.strftime("%Y-%m-%d %I:%M %p")
        if latest_dt_et
        else (str(timestamp_raw) if timestamp_raw else None)
    )
    ...
    raw_high_levels = cluster_levels(
        pivot_highs + [r["high"] for r in rows], bucket_width, min_touches=2, top_n=10
    )
    raw_low_levels = cluster_levels(
        pivot_lows + [r["low"] for r in rows], bucket_width, min_touches=2, top_n=10
    )
    ...
    if len(upper_levels) < 2:
        fallback_above = [
            lvl
            for lvl in resistance_candidates
            if lvl["price"] >= current and lvl not in upper_levels
        ]
        upper_levels.extend(fallback_above[: 2 - len(upper_levels)])
    if len(lower_levels) < 2:
        fallback_below = [
            lvl
            for lvl in sorted(support_candidates, key=lambda lvl: lvl["price"], reverse=True)
            if lvl["price"] <= current and lvl not in lower_levels
        ]
    ...
    current_day_key = (
        latest_dt_et.date().isoformat()
        if latest_dt_et
        else (ordered_days[-1] if ordered_days else None)
    )
    ...
    failed_up = count_failed_attempts(
        current_day_rows, resistance1["price"] if resistance1 else None, "up", bucket_width
    )
    failed_down = count_failed_attempts(
        current_day_rows, support1["price"] if support1 else None, "down", bucket_width
    )
    recent = recent_extremes(rows, window=min(20, len(rows)))
    volume_profile = compute_volume_profile(
        values, latest_dt_et, safe_float(latest.get("volume")), interval
    )
    channels = build_channels(current, support1, support2, resistance1, resistance2)
    probabilities = score_breakout(
        current,
        support1,
        resistance1,
    ...
    resistance_payloads = [
        as_level_payload(level, current, "resistance")
        for level in [resistance1, resistance2]
        if level
    ]
    support_payloads = [
        as_level_payload(level, current, "support") for level in [support1, support2] if level
    ]
    return {
        "ticker": ticker,
        "display": display,
        ...
        "previous_day": {
            ...
            "close": round_price(previous_day_summary.get("close"))
            if previous_day_summary
            else None,
            "volume": round_maybe(previous_day_summary.get("volume"), 2)
            if previous_day_summary
            else None,
        },
...
def run_breakout(
    ...
    for channel in output.get("channels", []):
        print(
            f"- {channel['name']}: ${channel['lower']:.2f} -> ${channel['upper']:.2f} ({channel['location']})"
        )
    print()

...
def fallback_relationships(target):
    ...
    for symbol in GROUP_FALLBACKS.get(target_group, []):
        ...
        if not sym or sym == target or sym in seen:
            continue
        selected.append(
            {"symbol": sym, "relation": "negative" if sym in NEGATIVE_MACRO else "positive"}
        )
        seen.add(sym)
        if len(selected) >= MAX_RELATIONSHIPS:
            return sanitize_relationships(target, selected)
...
def build_relationships_for_symbol(target, return_map):
    ...
    for other, series in return_map.items():
        ...
        if (
            target_group
            and other_group
            and target_group != other_group
            and score < CROSS_ASSET_MIN_SCORE
            and other not in NEGATIVE_MACRO
        ):
            continue
...
def remove_stale_lock(symbol):
    ...
    if (
        lock_path.exists()
        and (datetime.now().timestamp() - lock_path.stat().st_mtime) > LOCK_STALE_SECONDS
    ):
...
def acquire_lock(symbol):
    ...
    with os.fdopen(fd, "w") as f:
        f.write(
            json.dumps({"symbol": symbol, "started_at": utc_now_iso(), "pid": os.getpid()}) + "\n"
        )
...
def main(argv):
    ...
    # Acquire all per-symbol locks BEFORE doing any network work. Previously
    # the fetch_series loop ran for every requested symbol's peer basket up
    # front, then locks were checked at the end — meaning duplicate spawns
    # for the same ticker each blew ~50 HTTP requests at Massive before
    # discovering they were redundant. With load average peaking at 32 and
    # 15+ overlapping processes for popular tickers, that was the dominant
    # source of contention. Acquiring locks first lets duplicates exit
    ...
    for symbol in requested:
        ...
        if lock_path is not None:
            locked[symbol] = lock_path

    if not locked:
        # Every requested symbol is already being generated by another process.
        # Nothing to do; exit quietly so we don't pile up CPU/RAM.
        print(
            f"All {len(requested)} requested symbol(s) already locked by another "
            f"worker; exiting without fetching."
        )
        return

    try:
        # Mark every locked symbol as pending up front so api.php's
        # should_skip_correlation_spawn() can see that work is in flight.
        ...
        # Now do the expensive HTTP work — only for the symbols we actually
        # hold locks for. Same peer-basket assembly as before.
        return_map = {}
        fetch_symbols = set(locked.keys())
        for symbol in locked:
        ...
        fetch_symbols = sorted(
            normalize_symbol(s) for s in fetch_symbols if normalize_symbol(s)
        )
        ...
        # Always release every lock we acquired, even if the body raised.
        for lock_path in locked.values():

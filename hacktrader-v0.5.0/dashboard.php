<?php
session_start();
function hacktrader_version(): string {
    $path = __DIR__ . '/VERSION.md';
    if (!is_readable($path)) return 'v0.5.0';
    if (preg_match('/\*\*(v[^*]+)\*\*/', file_get_contents($path) ?: '', $m)) return $m[1];
    return 'v0.5.0';
}
$HACKTRADER_VERSION = hacktrader_version();
...
<head>
    ...
    <title>HackTrader | <?php echo htmlspecialchars($HACKTRADER_VERSION, ENT_QUOTES); ?> FUI</title>
...
+            text-align: right;
+        }
+        .trend-box .price-value {
+            font-size: 17px;
+        }
+        .trend-box.left .price-value {
+            color: var(--accent-red);
+        }
+        .trend-box.right .price-value {
...
<body onload='syncToleranceValue(); updateDashboard()'>
    ...
    <div id='disclaimerBanner' class='status-banner disclaimer-banner'>Predictive analysis maybe wrong, this is not trading advice</div>
    <div class='clock-container'>
        <div class='clock-shell'>
            ...
            <aside class='focus-panel'>
                <div class='side-trend-rail'>
                    <div class='trend-box left'>
                        <div class='price-label'>Downtrend</div>
                        <div class='price-value' id='downtrendBox'>--</div>
                        <div class='price-diff'>Focus ticker</div>
                    </div>
                    <div class='trend-box right'>
                        <div class='price-label'>Uptrend</div>
                        <div class='price-value' id='uptrendBox'>--</div>
                        <div class='price-diff'>Focus ticker</div>
                    </div>
                </div>
                <div class='top-rail'>
    ...
    <script>
        ...
        function updateFocusPanel(data, indicatorSummary = null) {
            ...
            const downtrendBox = document.getElementById('downtrendBox');
            const uptrendBox = document.getElementById('uptrendBox');
            if (downtrendBox) downtrendBox.textContent = `${Number(data?.probabilities?.down ?? 0)}%`;
            if (uptrendBox) uptrendBox.textContent = `${Number(data?.probabilities?.up ?? 0)}%`;
            ...
            const summary = indicatorSummary || data.indicator_summary || data.bias_summary || data.probabilities;
            if (summary && typeof summary.up !== 'undefined' && typeof summary.down !== 'undefined') {
    ...
    <footer style='position: fixed; bottom: 10px; width: 100%; text-align: center; font-size: 10px; color: #444;'><?php echo htmlspecialchars($HACKTRADER_VERSION, ENT_QUOTES); ?></footer>
</body>
</html>